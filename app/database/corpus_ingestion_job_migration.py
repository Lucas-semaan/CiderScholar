"""SQLite migration replacing private-ingestion jobs with corpus ingestion."""
...
def rename_private_ingestion_job_contract(connection: sqlite3.Connection) -> None:
    """Rebuild jobs so existing ingestion work remains queued in the common corpus."""
    ...
    connection.executescript(
        ...
        ALTER TABLE job_events RENAME TO job_events_v26;
        ALTER TABLE jobs RENAME TO jobs_v26;
        ...
        CREATE TABLE jobs (
            ...
            type TEXT NOT NULL CHECK(
                type IN (
                    ...
                    'long_synthesis', 'corpus_ingestion'
                )
            ),
            ...
            step TEXT NOT NULL CHECK(
                step IN (
                    ...
                    'evidence', 'verification', 'synthesis', 'ingestion'
                )
            ),
        ...
        SELECT
            id,
            CASE type
                WHEN 'private_ingestion' THEN 'corpus_ingestion'
                ELSE type
            END,
            state, step, payload_json, priority, attempt, available_at, worker_id,
            lease_expires_at, heartbeat_at, conversation_id, user_message_id,
            result_message_id, client_request_id, error_code, error_message,
        ...
        INSERT INTO job_events SELECT * FROM job_events_v26;
        DROP TABLE job_events_v26;
        DROP TABLE jobs_v26;
